...
import { Menu, X, User, MessageSquare, BarChart3, Trash2 } from "lucide-react";
...
export function Sidebar({ isOpen, onToggle, children, className = "" }: SidebarProps) {
  ...
  return (
    <>
      {/* Sidebar */}
      <div className={`${isOpen ? 'w-64' : 'w-0'} ${isOpen ? 'md:relative fixed' : ''} md:relative absolute top-0 left-0 h-full z-50 transition-all duration-300 overflow-hidden bg-gray-900 flex flex-col ${className}`}>
        {/* Brand Header */}
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" className="w-full justify-start p-2 h-auto hover:bg-gray-800">
                <UserAvatar profile={userProfile} size="sm" showName={true} className="text-white [&_span]:text-white" />
              </Button>
            </DropdownMenuTrigger>
